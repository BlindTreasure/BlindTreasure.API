...
# Tách từ script inline hiện tại và tối ưu hóa cho chạy độc lập trong CI
import pandas as pd
...
def base_function_from_test(method_name: str) -> str:
    """
    Lấy function-under-test từ test method.
    Ví dụ: RegisterCustomerAsync_ShouldReturnTrue_WhenValidData -> RegisterCustomerAsync
    """
...
def same_function(doc_func: str, test_method: str) -> bool:
    """Ghép function từ XML với function-under-test suy từ tên test. Bền với hậu tố Async, partial match."""
    d = norm(doc_func)
    ...
    if t.endswith('async') and d == t[:-5]:
        return True
    if len(d) > 3 and len(t) > 3 and (d in t or t in d):
        return True
    return False
...
def expected_from_name(name: str) -> str:
    ...
    if 'shouldthrow' in n or 'exception' in n or 'shouldthrownotfound' in n or 'shouldthrowconflict' in n or 'shouldthrowbadrequest' in n:
        return 'EXCEPTION'
    # vài pattern “thành công” thường gặp
    if 'shouldcreateuser' in n or 'shouldupdateprofile' in n or 'shoulduploadavatar' in n or 'shouldupdatestatus' in n or 'shouldreactivateuser' in n:
        return 'TRUE'
    return ''

def get_expected_return_value(name: str) -> str:
    n = (name or '').lower()
    if 'shouldcreateuser' in n:      return 'UserDto object'
    if 'shouldupdateprofile' in n:   return 'Updated UserDto'
    if 'shoulduploadavatar' in n:    return 'UserDto with avatar URL'
    if 'shouldupdatestatus' in n:    return 'Updated UserDto'
    if 'shouldreactivateuser' in n:  return 'Reactivated UserDto'
...
def extract_log_message_from_name(name: str) -> str:
    n = (name or '').lower()
    if 'conflict' in n or 'emailexists' in n:  return 'Email already exists'
    if 'forbidden' in n:                       return 'Forbidden access'
    if 'notfound' in n or 'usernotexists' in n:return 'User not found'
    if 'badrequest' in n or 'fileisempty' in n:return 'Invalid input data'
...
def to_conditions_from_name(name: str):
    ...
    if 'validfile' in base:
        conditions.append('Valid file provided')
    if 'empty' in base or 'null' in base:
        if 'file' in base:
            conditions.append('File is null or empty')
        elif 'name' in base:
    ...
    if 'emailexists' in base or 'conflict' in base:
        conditions.append('Email already exists')
    if 'userexists' in base:
        conditions.append('User exists in system')
    if 'statusisactive' in base:
        conditions.append('Status is set to Active')
    if not conditions:
        conditions.append('Standard test condition')
    return conditions[:4]  # tối đa 4 điều kiện để ma trận gọn

def safe_sheet_name(name, existing_sheets):
    """Tên sheet hợp lệ, ≤31 ký tự, tránh trùng."""
    if not name:
        name = "Sheet"
    safe_name = re.sub(r'[\\/*\[\]:?]', '_', str(name))
    max_base_length = 25  # chừa chỗ cho suffix " (n)"
...
def build_core():
    ...
    # Index TRX theo (Class, Base) với logic match “same_function”
...
def write_matrix(workbook, writer, func, tests):
    ...
    blue_header = workbook.add_format({
        ...
        "bg_color": "#1e1b4b", "font_color": "white"
    })
    ...
    for k, t in enumerate(tests, 1):
        ...
        cols.append({
            ...
            "type": classify_case(title),
            "out": t["Outcome"],
            "dt": t["ExecutedAt"],
            "conds": to_conditions_from_name(title),
            "log_msg": extract_log_message_from_name(title)
        })

    # tổng hợp điều kiện duy nhất
    all_conditions = []
    for c in cols:
        for cond in c["conds"]:
    ...
    for condition in all_conditions:
        ...
        for j, c in enumerate(cols):
            ws.write(r, 1 + j, "O" if condition in c["conds"] else "", data_format)
        r += 1
    ...
    ws.write(r, 0, "Expected Return Value", label_format)
    for j, c in enumerate(cols):
        ws.write(r, 1 + j, c["return_value"] or "", data_format)
    r += 1

    # FALSE / TRUE
    ws.write(r, 0, "FALSE", label_format)
    for j, c in enumerate(cols):
        ws.write(r, 1 + j, "O" if c["exp"] == "FALSE" else "", data_format)
    r += 1

    ws.write(r, 0, "TRUE", label_format)
    for j, c in enumerate(cols):
        ws.write(r, 1 + j, "O" if c["exp"] == "TRUE" else "", data_format)
    r += 1
    ...
    ws.write(r, 0, "Exception Expected", label_format)
    for j, c in enumerate(cols):
        ws.write(r, 1 + j, "O" if c["exp"] == "EXCEPTION" else "", data_format)
    r += 1
...
def main():
    ...
    # map tên sheet thực tế để cập nhật hyperlink
    sheet_name_mapping = {}
    ...
    with pd.ExcelWriter(out, engine="xlsxwriter") as w:
        ...
        # Matrix per function (chỉ tạo khi có test)
        for f in funcs:
            tests = idx.get((f["Class"], f["Function"]), [])
            if tests:
                actual_sheet = write_matrix(wb, w, f, tests)
                sheet_name_mapping[f["Function"]] = actual_sheet

        # Cập nhật hyperlink trong Statistics về đúng sheet thực tế
    ...
    # In tóm tắt ra log CI cho dễ trace
