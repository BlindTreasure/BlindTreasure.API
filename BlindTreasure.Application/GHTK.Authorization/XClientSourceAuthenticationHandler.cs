...
{
    public XClientSourceAuthenticationHandler(IOptionsMonitor<XClientSourceAuthenticationHandlerOptions> options,
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (clientSource.Count == 0) return AuthenticateResult.Fail("Missing X-Client-Source header");

        if (tokenHeader.Count == 0) return AuthenticateResult.Fail("Missing Token header");
    ...
    private bool VerifyClient(string clientSourceValue, string tokenValue, out SecurityToken? token,
        out ClaimsPrincipal? principal)
    {
        if (!Validate(tokenValue, out token, out principal)) return false;

        var sub = (token as JwtSecurityToken)!.Subject;

        if (clientSourceValue != sub) return false;
