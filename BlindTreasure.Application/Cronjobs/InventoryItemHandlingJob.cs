...
public class InventoryItemHandlingJob : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            {
                var nextRunVietnamTime =
                    TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow.Add(initialDelay), _vietnamTimeZone);
                _logger.LogInformation(
                    "InventoryItemHandlingJob scheduled to run at {NextRun} Vietnam time (delay: {DelayMinutes:F1} minutes)",
    ...
    private TimeSpan CalculateInitialDelay()
    {
        ...
        if (vietnamNow.Hour >= TARGET_HOUR) nextRunVietnam = nextRunVietnam.AddDays(1);
    ...
    private async Task ExecuteJobAsync()
    {
        ...
        _logger.LogInformation("Starting InventoryItemHandlingJob execution at {VietnamTime} Vietnam time",
            vietnamTime);
        ...
        try
        {
            ...
            foreach (var item in items) await inventoryService.HandleInventoryItemLifecycleAsync(item);
