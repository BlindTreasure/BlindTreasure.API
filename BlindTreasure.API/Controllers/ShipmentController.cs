...
namespace BlindTreasure.API.Controllers
{
    ...
    public class ShipmentController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMyShipments([FromQuery] Guid? orderId = null, [FromQuery] Guid? orderDetailId = null)
        {
            try
            {
        ...
        public async Task<IActionResult> GetByOrderDetailId(Guid orderDetailId)
        {
            try
            {
                ...
                return Ok(ApiResult<List<ShipmentDto>>.Success(result, "200", "Lấy shipment theo order detail thành công."));
            }
            catch (Exception ex)
            {
                var statusCode = ExceptionUtils.ExtractStatusCode(ex);
