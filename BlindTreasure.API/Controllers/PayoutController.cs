...
public class PayoutController : ControllerBase
{
    ...
    public PayoutController(IPayoutService payoutService, ILoggerService loggerService, IClaimsService claimsService,
        ...
        try
        {
            ...
            if (success == null)
                return BadRequest(ApiResult<object>.Failure("400",
                    "Không có payout hợp lệ hoặc doanh thu thực chưa đủ để rút."));

            return Ok(ApiResult<object>.Success(success, "200",
                "Yêu cầu rút tiền đã được gửi thành công. Chờ staff duyệt."));
        }
        catch (Exception ex)
        {
            _loggerService.Error($"[RequestPayout] {ex.Message}");
            return StatusCode(500,
                ApiResult<bool>.Failure("500", "Có lỗi xảy ra khi gửi yêu cầu rút tiền" + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> GetEligiblePayout()
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                ApiResult<object>.Failure("500", "Có lỗi xảy ra khi kiểm tra payout :" + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> ProcessSellerPayout(Guid sellerId)
    {
        try
        {
            ...
            if (!success)
                return BadRequest(ApiResult<bool>.Failure("400",
                    "Không thể thực hiện rút tiền. Kiểm tra lại điều kiện hoặc liên hệ hỗ trợ."));
    ...
    public async Task<IActionResult> CalculateUpcomingPayout()
    {
        try
        {
            ...
            return Ok(ApiResult<PayoutCalculationResultDto>.Success(result, "200",
                "Tính toán payout sắp tới thành công."));
        }
        catch (Exception ex)
        {
            _loggerService.Error($"[CalculateUpcomingPayout] {ex.Message}");
            return StatusCode(500,
                ApiResult<PayoutCalculationResultDto>.Failure("500",
                    "Có lỗi xảy ra khi tính toán payout." + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> GetSellerPayoutsForPeriod([FromBody] PayoutCalculationRequestDto req)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                ApiResult<List<PayoutListResponseDto>>.Failure("500",
                    "Có lỗi xảy ra khi lấy lịch sử payout." + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> GetPayoutDetailById(Guid payoutId)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                ApiResult<PayoutDetailResponseDto>.Failure("500",
                    "Có lỗi xảy ra khi lấy chi tiết payout." + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> ExportLatestPayoutProof()
    {
        try
        {
            ...
            var fileName = $"PayoutProof_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                ApiResult<object>.Failure("500", "Có lỗi xảy ra khi export file payout." + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> ExportPayoutsByPeriod(Guid payoutId)
    {
        try
        {
            ...
            var fileName = $"PayoutHistory_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
            var fileType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
            return File(stream, fileType, fileName);
        }
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                ApiResult<object>.Failure("500", "Có lỗi xảy ra khi export file payout history." + ex.Message));
        }
    }
    ...
    public async Task<IActionResult> GetMyPayouts([FromQuery] PayoutAdminQueryParameter param)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500,
                ApiResult<object>.Failure("500", "Có lỗi xảy ra khi lấy danh sách payouts của seller." + ex.Message));
        }
    }
}